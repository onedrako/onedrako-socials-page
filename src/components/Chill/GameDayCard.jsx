...
const GameDayCard = ({ gameDay }) => {
    ...
    const initialStreamSchedule = moment.tz(
        `${gameDay.date.substring(0, 10)}T${gameDay.schedule.initialTime}`,
        "America/Mexico_City"
    );

    const myStartStreamAreaInMexicoCity = moment.tz(
        `${gameDay.date.substring(0, 10)}T${gameDay.schedule.initialTime}`,
        "America/Mexico_City"
    );
    const myEndStreamAreaInMexicoCity = moment.tz(
        `${gameDay.date.substring(0, 10)}T${gameDay.schedule.endTime}`,
        "America/Mexico_City"
    );

    const dateForGameCard = moment
        .tz(initialStreamSchedule, userTimeZone.timeZone)
        .locale("es")
        .format("dd, DD/MMMM");

    const initialTimeForUserCountry = moment
        .tz(myStartStreamAreaInMexicoCity, userTimeZone.timeZone)
        .format("HH:mm");
    const endTimeForUserCountry = moment
        .tz(myEndStreamAreaInMexicoCity, userTimeZone.timeZone)
        .format("HH:mm");
            ...
            <GameDayCardContainer>
                <CardContainer status={status} shortName={gameDay.shortName}>
                    <img
                        ...
                        src={gameDay.game.largeImage}
                        alt={`Imagen de ${gameDay.game.name}`}
                    />
                    {gameDay.shortName === "ESP" ? (
                        <SpecialEventP>EVENTO ESPECIAL</SpecialEventP>
                    ) : null}
                    <MainInfoContainer>
                        ...
                        <div>
                            ...
                            {gameDay.sponsor && gameDay.Sponsor !== "null" ? (
                                <h3>{gameDay.sponsor}</h3>
                            ) : (
                                <h3>---</h3>
                            )}
                        </div>
                ...
                {extraInfo ? (
                    <ExtraInfo shortName={gameDay.shortName}>
                        ...
                        {gameDay.game.name === "Descansito" ? null : (
                            <>
                                <div>
                                    <h3>Plataformas donde juego: </h3>
                                    {gameDay.game.platforms.map(
                                        (platform, index) => (
                                            <PlatformsDiv
                                                key={index}
                                                img={platform.img}
